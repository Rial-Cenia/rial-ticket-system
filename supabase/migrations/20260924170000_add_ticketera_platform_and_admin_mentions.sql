alter type public."Platform" add value if not exists 'TICKETERA' after 'KAYS';
...
begin
  ...
  status_changed := previous.status is distinct from changed.status;
  ...
  if p_source = 'WEB' and changed."discordThreadId" is not null then
    if status_changed then
      ...
      content := case changed.status
        ...
        when 'RESUELTO' then format('🎉 **¡Caso cerrado, criaturas!** 🎉%sEl ticket `%s` fue marcado como **RESUELTO** por **%s** ✅💖%sEl problema fue derrotado, la paz regresó al reino y el team sirvió desarrollo con éxito 💅🏻✨%sCommon support W, besties ʕっ•ᴥ•̀ʔっ♡', E'\n', code, actor, E'\n', E'\n')
      ...
      if previous.platform is distinct from changed.platform then content := content || format(E'\n🖥️ Plataforma: **%s**.', coalesce(case changed.platform when 'NESTOR' then '🌸 Nestor' when 'DYLAN' then '⭐ Dylan' when 'ATOM' then '⚛️ Atom' when 'KAYS' then '🎀 Kays' when 'TICKETERA' then '🎟️ Ticketera' when 'EXTERNO' then '🌍 Externo' end, 'Sin asignar')); end if;
    end if;
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'content', content,
        'notifyTicketeraAdmins', status_changed and changed.platform = 'TICKETERA'
      )
    );
